...
//    )
//
//    // Vertex shader (identity transform)
//    private val vertexShaderCode = """
//        attribute vec4 vPosition;
//        attribute vec2 aTexCoord;
//        uniform mat4 uMVPMatrix;
...
//    """.trimIndent()
//
//    // Fragment shader (flip Y so image appears upright)
//    private val fragmentShaderCode = """
//        precision mediump float;
//        varying vec2 vTexCoord;
//        uniform sampler2D uTexture;
...
//        GLES20.glUseProgram(program)
//
//        // ✅ Fix: Reset vertex buffer before drawing
//        vertexBuffer.position(0)
//        GLES20.glEnableVertexAttribArray(positionHandle)
...
//        )
//
//        // Reset buffer for next frame (important!)
//        vertexBuffer.position(0)
//
//        GLES20.glUniformMatrix4fv(mvpMatrixHandle, 1, false, mvpMatrix, 0)
//        GLES20.glActiveTexture(GLES20.GL_TEXTURE0)
...
//    }
//
//    // Called from UI thread to update bitmap
//    fun updateTextureData(bitmap: Bitmap) {
...
class EdgeDetectionRenderer : GLSurfaceView.Renderer {
    ...
    // Full-screen quad (reference-matched: positions like ref, texture coords Y-flipped for upright)
    private val vertices = floatArrayOf(
        // Positions       // Texture Coords (Y-flipped like ref: bottom=texture top)
        -1f, -1f, 0f,      0f, 1f,   // Bottom Left → Texture Bottom-Left (0,1)
        1f, -1f, 0f,      1f, 1f,   // Bottom Right → Texture Bottom-Right (1,1)
        -1f,  1f, 0f,      0f, 0f,   // Top Left → Texture Top-Left (0,0)
        1f,  1f, 0f,      1f, 0f    // Top Right → Texture Top-Right (1,0)
    )

    // Vertex shader (identity like ref: gl_Position = vPosition)
    private val vertexShaderCode = """
        attribute vec4 vPosition;
        attribute vec2 aTexCoord;
        uniform mat4 uMVPMatrix;
        varying vec2 vTexCoord;
        void main() {
            gl_Position = uMVPMatrix * vPosition;  // Identity for full NDC
            vTexCoord = aTexCoord;
        }
    """.trimIndent()

    // Fragment shader (no Y-flip: coords handle it; linear for smooth edges)
    private val fragmentShaderCode = """
        precision mediump float;
        varying vec2 vTexCoord;
        uniform sampler2D uTexture;
        void main() {
            gl_FragColor = texture2D(uTexture, vTexCoord);  // Direct sample (coords fixed upright)
        }
    """.trimIndent()
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        GLES20.glGenTextures(1, textureId, 0)
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureId[0])
        // Smooth like ref: Linear for finesse (anti-aliased edges)
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, GLES20.GL_LINEAR)
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MAG_FILTER, GLES20.GL_LINEAR)
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_S, GLES20.GL_CLAMP_TO_EDGE)
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_T, GLES20.GL_CLAMP_TO_EDGE)

        Log.d(TAG, "OpenGL surface created (ref-matched coords, linear smooth)")
    }

    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        GLES20.glViewport(0, 0, width, height)
        Matrix.setIdentityM(mvpMatrix, 0)  // Ref-like: No transform, full viewport fill
        Log.d(TAG, "Viewport: ${width}x${height} - Camera-like NDC")
    }
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        if (bitmapToUse != null && needsUpdate.compareAndSet(true, false)) {
            ...
            bitmapToUse.recycle()
            newTextureBitmap = null
            Log.d(TAG, "✓ Texture updated: ${bitmapToUse.width}x${bitmapToUse.height} (camera-aligned)")
        }

        GLES20.glUseProgram(program)

        // Ref-matched: Position reset, pointers for interleaved buffer
        vertexBuffer.position(0)
        GLES20.glEnableVertexAttribArray(positionHandle)
        GLES20.glVertexAttribPointer(
            positionHandle,
        ...
        vertexBuffer.position(0)  // Reset for next
        ...
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4)  // Ref-like coverage
